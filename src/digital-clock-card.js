class DigitalClockCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      // Card configuration
      use_card: config.use_card !== undefined ? config.use_card : true,
  ...
  renderClock(hass) {
    ...
    let useCard = this.config.use_card;
    ...
    // Generate card wrapper based on use_card setting
    const cardWrapper = useCard ? 
      `<ha-card>
        <div style="display:flex; justify-content: ${justifyContent}; margin: ${marginDIV};">` : 
      `<div style="display:flex; justify-content: ${justifyContent}; margin: ${marginDIV};">`;
    
    const cardClosing = useCard ? `</div></ha-card>` : `</div>`;
...
class DigitalClockCardEditor extends HTMLElement {
  ...
  get _background_color() {
    return this.config.background_color || 'none';
  }
  ...
  get _width_svg() {
    return this.config.width_svg || '50%';
  }
  ...
  get _use_card() {
    return this.config.use_card !== undefined ? this.config.use_card : true;
  }
